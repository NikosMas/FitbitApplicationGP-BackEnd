...
public class FitbitCodeRequest {
	...
	public void codeRequest() throws IOException, InterruptedException {
		...
		final String URI = "https://www.fitbit.com/oauth2/authorize?" + "redirect_uri=http://localhost:8080"
				+ "&response_type=code" + "&client_id=227MLG"
				+ "&scope=activity%20nutrition%20heartrate%20profile%20settings%20sleep%20social%20weight"
				+ "&expires_in=2592000" + "&prompt=login";

		log.info(
				"-> WE JUST CREATE THE URI TO BE SENT TO THE FITBIT API WITH THE REQUIRED HEADERS AND SEND IT TO A BROWSER <-");

		Runtime openBrowser = Runtime.getRuntime();
		openBrowser.exec("rundll32 url.dll,FileProtocolHandler " + URI);
